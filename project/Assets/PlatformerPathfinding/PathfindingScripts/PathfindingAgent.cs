...
public class PathfindingAgent : MonoBehaviour {
    ...
    public Transform pathfindingTarget; /*Following / Chasing - overrides Vector3 Pathfinding*/
    ...
    public void CancelPathing() {
        ...
        if (endDrawComplete && pathLineRenderer) { pathLineRenderer.SetVertexCount(1); }
    ...
    private void PathStarted() {
        ...
        if (drawPath) {
            ...
            pathLineRenderer.SetColors(drawColor, drawColor);
            pathLineRenderer.SetVertexCount(currentOrders.Count);
    ...
    private void FixedUpdate() {
        ...
        if (pathIsDirty) {
            pathIsDirty = false;
            if (pathfindingTarget) { RequestPath(); } else if (hasLastOrder) { RequestPath(lastOrder); }
            if (debugBool) {
        ...
        if (currentOrders != null && !pathCompleted && debugBool) {
            for (int i = 0; i < currentOrders.Count; i++) {
                ...
                if (i == orderNum) {
                    ...
                    if (i == 0) { Gizmos.color = Color.green; } else if (i == currentOrders.Count - 1) { Gizmos.color = Color.red; } else {
    ...
    private void AddLineRenderer() {
        ...
        if (!GetComponent<LineRenderer>()) {
            ...
            pathLineRenderer.SetVertexCount(1);
            pathLineRenderer.SetWidth(0.1f, 0.1f);
        }
    }
}