...
public class Character : MonoBehaviour {
    ...
    void Awake() {
        ...
        _anim = transform.FindChild("Graphics").GetComponent<Animator>();
        _graphics = transform.FindChild("Graphics").gameObject; /*useful for preventing things from flipping when character is facing left*/
    ...
    void Update() {
        ...
        if (rightClickPathFind && Input.GetMouseButtonDown(1)) { //GetKeyDown(KeyCode.C)) {//
    ...
    void FixedUpdate() {
        ...
        //Jump sensitivity
        if (jump.ability && Input.GetKeyUp(KeyCode.Space)) { //think about adding an isCurrentlyJumping bool that gets reset to false on jetpack or landing or other forces affecting y
        ...
        if (jetpack.ability) {
            ...
            } else { _anim.SetBool("jetpack", false); jetpack.isJetpacking = false;
                if (!jetpack.hasFuel) {
        ...
        } else { // else if the character falls off an edge, 1 jump is lost
    ...
    public class movementEssentials {         /*gets inherited by movement abilities*/
        [System.NonSerialized]
        ...
        public void LedgeGrabState() {
            ...
            if (!_character.ladder.isClimbing && !ledgeGrabbed && !_character._controller.collisions.below && _character.velocity.y <= 0 && !ledgeCooldownBool) {
                ...
                if (!Physics2D.Raycast(position, direction, ledgeGrabDistance + _character.transform.localScale.x * _character._box.size.x * 0.5f, _character._controller.collisionMask)) {
                    ...
                    if (hit && hit.collider.tag != "oneway") { /*check collision beside character, if its oneway, ignore*/
                        ...
                        if (!vertHit || vertHit.collider.tag == "oneway") { /*check collision above character, if its oneway, ignore*/
