...
public class AiController : MonoBehaviour {
    ...
    */
    ...
    private void Awake() {
        ...
        _behaviourText = transform.FindChild("BehaviourText").GetComponent<TextMesh>();
        switch (state) {
    ...
    public void GetInput(ref Vector3 velocity, ref Vector2 input, ref bool jumpRequest) {
        if (_characterScript.ledgegrab.ledgeGrabbed) { _characterScript.ledgegrab.StopLedgeGrab(); } /*characters might get stuck on a ledge... this fixes that for now.*/
    ...
    private void PathfindChase() {
        ...
        if (PlayerInRange(6f, true)) {
            _pathAgent.pathfindingTarget = player.transform;
